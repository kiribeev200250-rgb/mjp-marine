...
export default async function RootLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <html lang="en">
      ...
      {/* Meta/TikTok/Google пиксели больше не грузятся здесь безусловно —
          раньше это затрагивало ВСЕ роуты, включая /crm и /admin, без какого
          -либо согласия пользователя. Теперь гейтятся через PixelGate/
          CookieConsent, подключённые точечно в app/(public)/layout.tsx и
          app/go/layout.tsx — только там, где они и нужны. */}
      <body>{children}</body>
    </html>
  );
}
